...
-(void)showDialoger
{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"单次定位" message:@"对系统定位进行了封装，同时可返回地址信息、网络状态信息和定位ID\n定位ID为：(cuid+idfa)经tp3加密后的字符串_时间戳" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)initBlock
{
    ...
    self.completionBlock = ^(BMKLocation *location, BMKLocationNetworkState state, NSError *error)
    {
        ...
        if (location.location) {//得到定位信息，添加annotation
            ...
            [strongSelf updateMessage:[NSString stringWithFormat:@"当前位置信息： \n经纬度：%.6f,%.6f \n地址信息：%@ \n网络状态：%d  \n定位ID：%@",location.location.coordinate.latitude, location.location.coordinate.longitude, [location.rgcData description], state, location.location]];
